...
export default function Header() {
  ...
  const isGuest =
    !profile || profile.uid === "guest" || profile.displayName === "Зочин";
      ...
      <div className="container mx-auto px-4 h-20 flex items-center justify-between max-w-6xl">
        <div className="flex items-center gap-6">
          ...
          {!isGuest && !hasPaid && (
            ...
            >
              <span className="w-1.5 h-1.5 bg-red-500 rounded-full" /> Идэвхгүй
              (15,000₮)
            </Link>
          )}
          ...
          {!isGuest && (
            <div className="flex items-center gap-2.5 sm:gap-6 text-xs sm:text-sm font-black">
              <div
                ...
                title={`${profile?.currentStreak || 0} хоног дараалсан streak`}
              >
                <span className="text-base sm:text-xl">🔥</span>
                <span>
                  {profile?.currentStreak || 0}
                  <span className="hidden sm:inline"> хоног</span>
                </span>
              </div>
              <div
                className="flex items-center gap-1 text-blue-600"
                title={`${profile?.totalXp || 0} XP`}
              >
                <span className="text-base sm:text-xl">💎</span>
                <span>
                  {profile?.totalXp || 0}
              ...
              <div
                className="px-2 sm:px-4 py-1.5 bg-bg-soft border border-border-sub rounded-full text-[9px] sm:text-xs font-black text-muted uppercase tracking-wider"
                title={`HSK ${profile?.selectedLevel || 1} Түвшин`}
              >
                <span className="hidden xs:inline">HSK </span>
