...
import { getHSKProgress, updateHSKProgress, saveUserProfile, saveFeedback } from '../lib/db';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <span>
          Та үнэгүй 1 өдрийн туршилтын хугацаатай байна. {timeLeft.hours} цаг {timeLeft.minutes} минут үлдлээ.
        </span>
      </div>
      <Link 
        to="/payment"
...
export default function Dashboard() {
  ...
  const [currentProgress, setCurrentProgress] = useState<HSKProgress | null>(null);
  ...
  useEffect(() => {
    if (profile) {
      getHSKProgress(profile.uid, profile.selectedLevel).then(setCurrentProgress);
    }
  ...
  const currentTip = levelTips.length > 0 ? levelTips[dayIndex % levelTips.length] : null;
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 sm:gap-8">
          <div className="space-y-2 w-full">
            <h2 className="text-2xl sm:text-3xl font-black text-ink tracking-tight flex flex-wrap items-center gap-2">
              Сайн байна уу, {profile?.displayName}! <span className="animate-bounce">👋</span>
            </h2>
            <p className="text-ink/60 text-sm sm:text-base font-medium">Күнзийн хэл сурах аялалаа үргэлжлүүлцгээе.</p>
            
            <div className="flex flex-wrap gap-2.5 mt-4 sm:mt-6">
              <StatBadge icon={<Flame className="w-4 h-4 text-orange-500 fill-orange-500" />} label={`${profile?.currentStreak || 0} өдөр`} />
              <StatBadge icon={<Trophy className="w-4 h-4 text-yellow-500 fill-yellow-500" />} label={`${profile?.totalXp || 0} XP`} />
              <StatBadge icon={<Target className="w-4 h-4 text-blue-500" />} label={`HSK ${profile?.selectedLevel}`} />
            </div>
          </div>
          
          <div className="w-full md:w-auto flex flex-col gap-2 shrink-0">
            <span className="text-[10px] font-black text-ink/40 uppercase tracking-wider md:hidden">Түвшин сонгох:</span>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            <Link to="/vocab" className="group">
              <div className="bg-white p-8 rounded-[32px] border border-border-sub shadow-sm hover:border-primary-orange transition-all h-full">
                ...
                <p className="text-ink/60 text-sm mb-6 leading-relaxed">HSK {profile?.selectedLevel} түвшний шаардлагатай бүх үгс.</p>
                <div className="space-y-3">
                  <ProgressBar label="Суралцаж буй" progress={currentProgress?.vocabularyCompleted || 0} max={100} color="bg-primary-orange" />
                </div>
              </div>
            </Link>
            ...
            <Link to="/grammar" className="group">
              <div className="bg-white p-8 rounded-[32px] border border-border-sub shadow-sm hover:border-emerald-500 transition-all h-full">
                ...
                <p className="text-ink/60 text-sm mb-6 leading-relaxed">Дүрэм болон өгүүлбэрийн бүтэц.</p>
                <div className="space-y-3">
                  <ProgressBar label="Явц" progress={currentProgress?.grammarCompleted || 0} max={100} color="bg-emerald-500" />
                </div>
              </div>
            </Link>
          </div>
            ...
            <div className="bg-white p-6 rounded-[24px] border border-border-sub flex items-center gap-5">
              ...
              <div className="flex-1">
                ...
                <div className="w-full bg-blue-100 h-1 rounded-full mt-2 overflow-hidden">
                  <div className="bg-blue-500 h-full" style={{ width: `${currentProgress?.listeningCompleted || 0}%` }} />
                </div>
              </div>
              <Link to="/listening" className="w-10 h-10 bg-bg-soft rounded-xl flex items-center justify-center text-ink/40 hover:text-primary-orange transition-colors">
            ...
            <div className="bg-white p-6 rounded-[24px] border border-border-sub flex items-center gap-5">
              ...
              <div className="flex-1">
                ...
                <div className="w-full bg-purple-100 h-1 rounded-full mt-2 overflow-hidden">
                  <div className="bg-purple-500 h-full" style={{ width: `${currentProgress?.readingCompleted || 0}%` }} />
                </div>
              </div>
              <Link to="/reading" className="w-10 h-10 bg-bg-soft rounded-xl flex items-center justify-center text-ink/40 hover:text-primary-orange transition-colors">
                <ChevronRight className="w-6 h-6" />
              </Link>
            </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-[32px] border border-border-sub shadow-sm">
            ...
            {currentTip ? (
              <div className="space-y-4">
                <div className="text-4xl font-black text-primary-orange">{currentTip.characters[0].hanzi}</div>
                <p className="text-ink/70 text-sm font-medium line-clamp-3">{currentTip.title}</p>
                <button 
              ...
              <p className="text-xs text-ink/40">Өнөөдөртөө зөвлөгөө алга байна.</p>
            )}
          </div>

          <Link to="/leaderboard" className="block bg-ink p-8 rounded-[32px] text-white overflow-hidden relative group">
            ...
            <p className="text-white/60 text-sm mb-6">Бусадтай өрсөлдөн оноогоо ахиул.</p>
...
function ProgressBar({ label, progress, max, color }: { label: string, progress: number, max: number, color: string }) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        <div className="p-8 border-b border-border-sub flex items-center justify-between bg-bg-soft">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-ink/40 font-bold uppercase tracking-widest">HSK {tip.level} зөвлөгөө</p>
            </div>
          </div>
          ...
          {tip.characters.map((char, i) => (
            <div key={i} className="space-y-4">
              <div className="flex items-center gap-6">
                <div className="text-6xl font-black text-primary-orange">{char.hanzi}</div>
                <div>
              ...
              <div className="grid gap-2">
                {char.examples.map((ex, ei) => (
                  <div key={ei} className="p-4 bg-bg-soft rounded-2xl flex items-center justify-between">
        ...
        <div className="p-6 bg-bg-soft border-t border-border-sub">
          <button onClick={onClose} className="w-full py-4 bg-ink text-white rounded-2xl font-bold">Ойлголоо</button>
        </div>
      </motion.div>
    </div>
  );
...
function FeedbackModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await saveFeedback(profile.uid, profile.email || '', message.trim(), rating);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Үнэлгээ:</label>
                ...
                >
                  <Star
                    className={`w-7 h-7 transition-colors ${
                      star <= rating ? 'fill-amber-400 text-amber-400' : 'text-slate-205 fill-transparent'
                    }`}
                  />
                </button>
              ))}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Санал, сэтгэгдэл:</label>
            <textarea
              value={message}
