...
import { HSKLevel, UserProfile, WordMastery, HSKProgress, OperationType } from '../types';
...
export const getUserProfile = async (uid: string): Promise<UserProfile | null> => {
  ...
  try {
    ...
    console.warn('getUserProfile Firestore error, checking offline cache:', error);
    try {
      const cached = localStorage.getItem(`cached_profile_${uid}`);
      if (cached) {
        console.log('Successfully resolved user profile from offline localStorage cache.');
    ...
    if (isOffline) {
      console.warn('Client is offline and no cached profile found. Returning default offline profile.');
      return {
        uid,
...
export const getLocalUserByEmail = async (email: string): Promise<UserProfile | null> => {
  const path = 'users';
  try {
    const q = query(
      collection(db, 'users'),
    ...
    const isOffline = error?.message?.includes('offline') || !navigator.onLine;
    if (isOffline) {
      console.warn('Cannot perform search by email while offline, returning null.');
      return null;
    }
    handleFirestoreError(error, OperationType.GET, path);
    return null;
...
export const saveUserProfile = async (uid: string, profile: Partial<UserProfile>) => {
  if (!uid || uid === 'undefined') {
    console.warn('saveUserProfile was called with invalid or undefined uid:', uid);
    return;
  }
  const path = `users/${uid}`;
  try {
    ...
    if (profile.displayName !== undefined) dbPayload.displayName = profile.displayName;
    
    if (profile.selectedLevel !== undefined) {
      dbPayload.selectedLevel = typeof profile.selectedLevel === 'number' 
    ...
    if (profile.createdAt !== undefined) dbPayload.createdAt = profile.createdAt;
    ...
    if (profile.trialEligibilityCheckFailed !== undefined) {
      dbPayload.trialEligibilityCheckFailed = profile.trialEligibilityCheckFailed;
    }
    ...
    try {
      ...
      const merged = {
        ...
        totalXP: dbPayload.totalXP !== undefined ? dbPayload.totalXP : (existing.totalXP !== undefined ? existing.totalXP : dbPayload.totalXp),
        totalXp: dbPayload.totalXp !== undefined ? dbPayload.totalXp : (existing.totalXp !== undefined ? existing.totalXp : dbPayload.totalXP),
        streak: dbPayload.streak !== undefined ? dbPayload.streak : (existing.streak !== undefined ? existing.streak : dbPayload.currentStreak),
        currentStreak: dbPayload.currentStreak !== undefined ? dbPayload.currentStreak : (existing.currentStreak !== undefined ? existing.currentStreak : dbPayload.streak),
        isPaid: dbPayload.isPaid !== undefined ? dbPayload.isPaid : existing.isPaid,
        isActive: dbPayload.isActive !== undefined ? dbPayload.isActive : existing.isActive,
      };
    ...
    const isOffline = error?.message?.includes('offline') || !navigator.onLine;
    if (isOffline) {
      console.log('Saved to local storage successfully. App operation is offline-ready.');
      return;
    }
...
export const updateStudyStreak = async (uid: string) => {
  ...
  try {
    ...
    let newStreak = user.streak !== undefined ? user.streak : (user.currentStreak !== undefined ? user.currentStreak : 0);
    ...
    try {
      ...
      console.warn('Failed to update streak in Firestore (offline?):', firebaseErr);
      const isOffline = firebaseErr?.message?.includes('offline') || !navigator.onLine;
      if (!isOffline) {
        throw firebaseErr;
      }
    }
...
export const getWordMastery = async (uid: string): Promise<Record<string, WordMastery>> => {
  if (!uid) return {};
...
export const updateWordMastery = async (uid: string, wordId: string, isCorrect: boolean) => {
  if (!uid) return;
  ...
  try {
    ...
    try {
      ...
      console.warn('Failed to save word mastery to firestore (offline?):', firebaseErr);
      const isOffline = firebaseErr?.message?.includes('offline') || !navigator.onLine;
      if (!isOffline) {
        throw firebaseErr;
      }
...
export const getHSKProgress = async (uid: string, level: HSKLevel): Promise<HSKProgress | null> => {
  if (!uid) return null;
  ...
  try {
    ...
    if (pDoc.exists()) {
      ...
      try {
        localStorage.setItem(`cached_progress_${uid}_${levelKey}`, JSON.stringify(progress));
      } catch (e) {}
      return progress;
    }
    return null;
...
export const updateHSKProgress = async (uid: string, level: HSKLevel, updates: Partial<HSKProgress>) => {
  if (!uid) return;
  const levelKey = `hsk${level}`;
  const path = `users/${uid}/progress/${levelKey}`;
  try {
    ...
    try {
      localStorage.setItem(`cached_progress_${uid}_${levelKey}`, JSON.stringify(nextData));
    } catch (e) {}
    ...
    try {
      ...
      console.warn('Failed to save HSK progress to firestore (offline?):', firebaseErr);
      const isOffline = firebaseErr?.message?.includes('offline') || !navigator.onLine;
      if (!isOffline) {
        throw firebaseErr;
      }
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      list.push({
        ...
        weeklyXp: u.totalXp !== undefined ? u.totalXp : (u.xp !== undefined ? u.xp : 0),
        rank: rank++
      });
    });
...
export const subscribeToPendingPayments = (
  ...
  const parseDocToProfile = (document: any): any => {
    ...
    return {
      ...
      isActive: isPaidUser,
      expiryDate: data.expiryDate || data.paidUntil || null,
      paymentSubmittedAt: data.paymentSubmittedAt || (data.createdAt ? (typeof data.createdAt.toDate === 'function' ? data.createdAt.toDate().toISOString() : data.createdAt) : null),
    };
  };
  ...
  try {
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.warn('Query with orderBy failed (maybe index is building). Fallback to standard query without orderby:', error);
        ...
        const unsubFallback = onSnapshot(
          ...
          (snapshot) => {
            ...
            list.sort((a, b) => {
              const timeA = new Date(a.paymentSubmittedAt || a.createdAt || 0).getTime();
              const timeB = new Date(b.paymentSubmittedAt || b.createdAt || 0).getTime();
              return timeB - timeA;
            });
            onData(list);
          },
          (fallbackErr) => {
            console.error('All onSnapshot subscription queries failed:', fallbackErr);
            onError(fallbackErr);
          }
        );
    ...
    console.error('Failed to set up onSnapshot in subscribeToPendingPayments:', err);
    onError(err);
    return () => {};
  }
};

export const saveFeedback = async (userId: string, email: string, message: string, rating: number) => {
...
export const subscribeToFeedbacks = (
  ...
  try {
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.warn('Query feedbacks with orderBy failed, falling back to unordered client-side sorted query:', error);
        ...
        const unsubFallback = onSnapshot(
          ...
          (fallbackErr) => {
            console.error('All onSnapshot subscription queries failed for feedbacks:', fallbackErr);
            onError(fallbackErr);
          }
        );
        (unsubscribe as any).fallback = unsubFallback;
...
export const getCompletedGrammarLessons = async (uid: string): Promise<string[]> => {
  if (!uid) return [];
  const path = `users/${uid}/grammarProgress`;
  try {
    const colRef = collection(db, 'users', uid, 'grammarProgress');
    const snapshot = await getDocs(colRef);
    const completedList: string[] = [];
    snapshot.forEach((doc) => {
      if (doc.data()?.completed) {
        completedList.push(doc.id);
      }
    });
    return completedList;
  } catch (error: any) {
    console.warn('getCompletedGrammarLessons error, reading local cache:', error);
    try {
      const cached = localStorage.getItem(`cached_grammar_completed_${uid}`);
      if (cached) {
        return JSON.parse(cached);
      }
    } catch (e) {}
...
export const completeGrammarLesson = async (uid: string, level: number, lessonId: string) => {
  if (!uid) return 0;
  const progressPath = `users/${uid}/grammarProgress/${lessonId}`;
  try {
    let completedList = await getCompletedGrammarLessons(uid);
    if (!completedList.includes(lessonId)) {
      completedList.push(lessonId);
    }
    
    try {
      localStorage.setItem(`cached_grammar_completed_${uid}`, JSON.stringify(completedList));
    } catch (e) {}

    const docRef = doc(db, 'users', uid, 'grammarProgress', lessonId);
    await setDoc(docRef, {
      completed: true,
      completedAt: new Date().toISOString()
    });

    const levelPrefix = `hsk${level}_`;
    const completedForLevel = completedList.filter(id => id.startsWith(levelPrefix)).length;
    const totalForLevel = 10;
    const percentage = Math.min(100, Math.round((completedForLevel / totalForLevel) * 100));

    await updateHSKProgress(uid, level as any, {
      grammarCompleted: percentage
    });
    ...
    console.warn('completeGrammarLesson error (offline?):', error);
    const isOffline = error?.message?.includes('offline') || !navigator.onLine;
    if (isOffline) {
      return 0;
    }
...
export const getCompletedListeningLessons = async (uid: string): Promise<string[]> => {
  if (!uid) return [];
  const path = `users/${uid}/listeningProgress`;
  try {
    const colRef = collection(db, 'users', uid, 'listeningProgress');
    const snapshot = await getDocs(colRef);
    const completedList: string[] = [];
    snapshot.forEach((doc) => {
      if (doc.data()?.completed) {
    ...
    console.warn('getCompletedListeningLessons error, reading local cache:', error);
    try {
      const cached = localStorage.getItem(`cached_listening_completed_${uid}`);
      if (cached) {
        return JSON.parse(cached);
      }
    } catch (e) {}
...
export const completeListeningLesson = async (uid: string, level: number, lessonId: string): Promise<number> => {
  if (!uid) return 0;
  const progressPath = `users/${uid}/listeningProgress/${lessonId}`;
  try {
    let completedList = await getCompletedListeningLessons(uid);
    if (!completedList.includes(lessonId)) {
      completedList.push(lessonId);
    }
    
    try {
      localStorage.setItem(`cached_listening_completed_${uid}`, JSON.stringify(completedList));
    } catch (e) {}

    const docRef = doc(db, 'users', uid, 'listeningProgress', lessonId);
    await setDoc(docRef, {
      completed: true,
      completedAt: new Date().toISOString()
    });
    ...
    console.warn('completeListeningLesson error (offline?):', error);
    const isOffline = error?.message?.includes('offline') || !navigator.onLine;
    if (isOffline) {
      return 0;
    }
...
export const getCompletedReadingLessons = async (uid: string): Promise<string[]> => {
  if (!uid) return [];
  const path = `users/${uid}/readingProgress`;
  try {
    const colRef = collection(db, 'users', uid, 'readingProgress');
    const snapshot = await getDocs(colRef);
    const completedList: string[] = [];
    snapshot.forEach((doc) => {
      if (doc.data()?.completed) {
    ...
    console.warn('getCompletedReadingLessons error, reading local cache:', error);
    try {
      const cached = localStorage.getItem(`cached_reading_completed_${uid}`);
      if (cached) {
        return JSON.parse(cached);
      }
    } catch (e) {}
...
export const completeReadingLesson = async (uid: string, level: number, lessonId: string): Promise<number> => {
  if (!uid) return 0;
  const progressPath = `users/${uid}/readingProgress/${lessonId}`;
  try {
    let completedList = await getCompletedReadingLessons(uid);
    if (!completedList.includes(lessonId)) {
      completedList.push(lessonId);
    }
    
    try {
      localStorage.setItem(`cached_reading_completed_${uid}`, JSON.stringify(completedList));
    } catch (e) {}

    const docRef = doc(db, 'users', uid, 'readingProgress', lessonId);
    await setDoc(docRef, {
      completed: true,
      completedAt: new Date().toISOString()
    });
    ...
    console.warn('completeReadingLesson error (offline?):', error);
    const isOffline = error?.message?.includes('offline') || !navigator.onLine;
    if (isOffline) {
      return 0;
    }
